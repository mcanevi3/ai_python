import numpy as np 
from controller import *
import matplotlib.pyplot as plt
import control

Fs=Controller()
Fs.load()
Fs.print()

samples=1001
t=np.linspace(0,20,samples)
T=t[2]-t[1]

x=np.zeros((samples,3))
u=np.ones((samples,1))
y=np.zeros((samples,1))
Ad = torch.eye(3) + T * A
Bd = T * B
Brd = T * Br


for i in range(1, samples):
    xi = torch.tensor(x[i-1], dtype=torch.float32)        # shape: (3,)
    # ui = Fs(xi.unsqueeze(0)).detach().numpy()[0, 0]       # scalar output

    ui= K @ xi.numpy()  # Apply feedback control
    u[i-1] = ui

    # Compute dx/dt = A x + B u
    dx = Ad @ xi + Bd.flatten() * ui  + Brd.flatten() * 1  # shape: (3,)
    x[i] = dx.numpy()
    y[i]=(C @ xi).numpy() # shape: (1,)
# Print final state
print("Initial state:", x[0])
print("Final state:", x[-1])

plt.figure(figsize=(12,4))
plt.plot(t,y,label='y')
plt.title("Learned Control Law u(x)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
